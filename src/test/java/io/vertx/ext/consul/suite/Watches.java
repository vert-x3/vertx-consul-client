...
public class Watches extends ConsulTestBase {
  ...
  public void clientWithTimeout(TestContext tc) {
    ...
    assertTrue(getAsync((h) -> writeClient.putValue(key, v3, h)));
    latch3.await(10_000);
  ...
  public void watchCreatedKey(TestContext tc) {
    ...
    tc.assertTrue(getAsync((h) -> writeClient.putValue(key, v1, h)));
    firstValue.await(2000);
    tc.assertEquals(v1, current.get());

    tc.assertTrue(getAsync((h) -> writeClient.putValue(key, v2, h)));
  ...
  public void testKeyPrefix(TestContext tc) {
    ...
    tc.assertTrue(getAsync((h) -> writeClient.putValue(k1, v1, h)));
    ...
    tc.assertTrue(getAsync((h) -> writeClient.putValue(k2, v2, h)));
    secondKey.await(500);
    tc.assertEquals(Stream.of(v1, v2).sorted().collect(Collectors.joining("/")), current.get());

    runAsync((h) -> writeClient.deleteValues(keyPrefix, h));
    emptyKey.countDown();
  ...
  public void iss70(TestContext tc) {
    ...
    vertx.setTimer(TimeUnit.SECONDS.toMillis(15), th -> {
      writeClient.putValue(key, v2, ignore -> {
      });
    });
  ...
  public void watchEvents(TestContext tc) {
    ...
    Utils.<Event>getAsync(h -> writeClient.fireEventWithOptions(
  ...
  public void watchNodeHealthChecks(TestContext tc) {
    ...
    failed.await(21_000);
    watch.stop();
    runAsync(h -> writeClient.deregisterService(opts.getId(), h));
  }
