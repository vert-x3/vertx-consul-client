...
public class Events extends ConsulTestBase {
  ...
  private void runTest(TestContext tc, boolean timeout) {
    ...
    writeClient.fireEventWithOptions(name1, opts, tc.asyncAssertSuccess(event -> {
      tc.assertEquals(name1, event.getName());
      tc.assertEquals(opts.getPayload(), event.getPayload());
      String evId1 = event.getId();
      writeClient.listEvents(tc.asyncAssertSuccess(list1 -> {
        long cnt1 = list1.getList().stream()
          .map(Event::getId)
          .filter(id -> id.equals(evId1))
          .count();
        tc.assertEquals(cnt1, (long) 1);
        writeClient.listEventsWithOptions(new EventListOptions().setName(name2), tc.asyncAssertSuccess(list2 -> {
          ...
          writeClient.listEventsWithOptions(new EventListOptions().setBlockingOptions(blockingQueryOptions), tc.asyncAssertSuccess(h -> {
          ...
          vertx.setTimer(4000, l -> {
            ...
            writeClient.fireEvent(name2, tc.asyncAssertSuccess(ev -> {
              writeClient.listEventsWithOptions(new EventListOptions().setName(name2), tc.asyncAssertSuccess(list3 -> {
