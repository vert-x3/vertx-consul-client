...
public class KVStore extends ConsulTestBase {
  ...
  public void handleExceptions() {
    ...
    readClient.getValue(unknownKey, kv -> {
      latch.countDown();
  ...
  public void readClientCantWriteOneValue(TestContext tc) {
    readClient.putValue("foo/bar1", "value1", tc.asyncAssertFailure());
  }
    ...
    writeClient
      .putValue(key, "", tc.asyncAssertSuccess(b -> {
        ...
        readClient.getValue(key, tc.asyncAssertSuccess(pair -> {
          ...
          writeClient.deleteValue(key, tc.asyncAssertSuccess());
        }));
      }));
  }
  ...
  private void valuesAccess(TestContext tc, ConsulClient accessClient) {
    ...
    writeClient.putValue(key1, value1, tc.asyncAssertSuccess(b1 -> {
      tc.assertTrue(b1);
      writeClient.putValue(key2, value2, tc.asyncAssertSuccess(b2 -> {
        tc.assertTrue(b2);
  ...
  public void checkAndSet(TestContext tc) {
    ...
    writeClient
      .putValue(key, randomAlphaString(10), tc.asyncAssertSuccess(b1 -> {
        tc.assertTrue(b1);
        readClient.getValue(key, tc.asyncAssertSuccess(pair1 -> {
          writeClient.putValue(key, randomAlphaString(10), tc.asyncAssertSuccess(b2 -> {
            tc.assertTrue(b2);
            readClient.getValue(key, tc.asyncAssertSuccess(pair2 -> {
              tc.assertTrue(pair1.getModifyIndex() < pair2.getModifyIndex());
              writeClient.putValueWithOptions(key, randomAlphaString(10), new KeyValueOptions().setCasIndex(pair1.getModifyIndex()), tc.asyncAssertSuccess(b3 -> {
                tc.assertFalse(b3);
                writeClient.putValueWithOptions(key, randomAlphaString(10), new KeyValueOptions().setCasIndex(pair2.getModifyIndex()), tc.asyncAssertSuccess(b4 -> {
                  tc.assertTrue(b4);
                  writeClient.deleteValue(key, tc.asyncAssertSuccess());
                }));
              }));
  ...
  public void clientCantDeleteUnknownKey(TestContext tc) {
    readClient.deleteValue("unknown", tc.asyncAssertFailure());
  }

  @Test
  public void canGetValueBlocking(TestContext tc) {
    blockingQuery(tc, (key, h) -> readClient.getValue(key, tc.asyncAssertSuccess(list -> h.handle(list.getModifyIndex()))));
  }

  @Test
  public void canGetValuesBlocking(TestContext tc) {
    blockingQuery(tc, (key, h) -> readClient.getValues(key, tc.asyncAssertSuccess(list -> h.handle(list.getIndex()))));
  }
  ...
  public void canGetKeysList(TestContext tc) {
    ...
    writeClient
      .putValue(key, randomAlphaString(10), tc.asyncAssertSuccess(b -> {
        tc.assertTrue(b);
        readClient.getKeys(keyPrefix, tc.asyncAssertSuccess(list -> {
          tc.assertTrue(list.contains(key));
          writeClient.deleteValues(keyPrefix, tc.asyncAssertSuccess());
        }));
      }));
  }
      ...
      .putValue(key, randomAlphaString(10), tc.asyncAssertSuccess(b1 -> {
        ...
        indexSupplier.accept(key, consulIndex -> {
          ...
          vertx.setTimer(TimeUnit.SECONDS.toMillis(2), l -> {
            writeClient.putValue(key, value, tc.asyncAssertSuccess(b2 -> {
              tc.assertTrue(b2);
              readClient.getValueWithOptions(key, new BlockingQueryOptions().setIndex(consulIndex), tc.asyncAssertSuccess(kv -> {
                tc.assertTrue(kv.getModifyIndex() > consulIndex);
                tc.assertEquals(kv.getValue(), value);
                async.countDown();
              }));
              readClient.getValuesWithOptions("foo/bar", new BlockingQueryOptions().setIndex(consulIndex), tc.asyncAssertSuccess(kv -> {
          ...
          async.handler(v -> writeClient.deleteValue(key, tc.asyncAssertSuccess()));
        });
      }));
  }
}
