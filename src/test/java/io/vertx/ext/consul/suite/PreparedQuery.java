...
public class PreparedQuery extends ConsulTestBase {
  ...
  public void checkDefinition(TestContext tc) {
    writeClient.createSession(tc.asyncAssertSuccess(sessId -> {
      PreparedQueryDefinition expected = randomPreparedQueryDefinition()
        ...
        .setSession(sessId);
      writeClient.createPreparedQuery(expected, tc.asyncAssertSuccess(id -> {
        readClient.getPreparedQuery(id, tc.asyncAssertSuccess(actual -> {
          ...
          writeClient.deletePreparedQuery(actual.getId(), tc.asyncAssertSuccess(v -> {
            writeClient.destroySession(expected.getSession(), tc.asyncAssertSuccess());
          }));
        }));
      }));
    }));
  ...
  public void execute(TestContext tc) {
    PreparedQueryDefinition def = new PreparedQueryDefinition()
      ...
      .setTemplateType("name_prefix_match")
      .setTemplateRegexp("^find_(.+?)_(.+?)$");
    writeClient.createPreparedQuery(def, tc.asyncAssertSuccess(qid -> {
      writeClient.executePreparedQuery("find_1_2", tc.asyncAssertSuccess(resp1 -> {
